...
#define IDM_WINDOW_FONT_7		217
	 ...
	 * Start TomeTik 0.3
	 */
#define IDM_WINDOW_PERF_0		220
#define IDM_WINDOW_PERF_1		221
#define IDM_WINDOW_PERF_2		222
#define IDM_WINDOW_PERF_3		223
#define IDM_WINDOW_PERF_4		224
#define IDM_WINDOW_PERF_5		225
#define IDM_WINDOW_PERF_6		226
#define IDM_WINDOW_PERF_7		227
	/*
	 * End TomeTik 0.3
	 */
...
#define IDM_OPTIONS_ASCII_GRAPHICS  403
	 ...
	 * Start TomeTik 0.3
	 */
#define IDM_OPTIONS_GERVAIS_GRAPHICS	404
	/*
	 * End TomeTik 0.3
	 */
...
static void save_prefs_aux(term_data *td, cptr sec_name)
{
	 ...
	 */
	/* Perfect */
	strcpy(buf, td->perfect ? "1" : "0");
	WritePrivateProfileString(sec_name, "Perfect", buf, ini_file);
	/*
	 * End TomeTik 0.3
	 */
...
static void save_prefs(void)
{
	 ...
	 * Start TomeTik 0.3
	 */
	/* Save the "arg_zoom" flag */
	sprintf(buf, "%d", arg_zoom);
	WritePrivateProfileString("Angband", "Zoom", buf, ini_file);
	/*
	 * End TomeTik 0.3
	 */
...
static void load_prefs_aux(term_data *td, cptr sec_name)
{
	 ...
	 */
	/* Perfect */
	td->perfect = (GetPrivateProfileInt(sec_name, "Perfect", td->perfect, ini_file) != 0);
	/*
	 * End TomeTik 0.3
	 */
...
static bool init_graphics()
{
	...
	/*if (can_use_graphics != arg_graphics) */
	{
		...
		else if (arg_graphics  == 3)
		{
			path_build(buf, 1024, ANGBAND_DIR_XTRA_GRAF, "mask32.bmp");
			/* Load the bitmap or quit */
			if (!ReadDIB(data[0].w, buf, &infMask))
			{
				plog_fmt("Cannot read bitmap file '%s'", name);
...
static void term_change_font(term_data *td)
{
	 ...
	 */
		if (td->perfect)
		{
			if (td->font_wid <= 5 && td->font_hgt <= 9)
			{
				td->tile_wid = 4;
				td->tile_hgt = 8;
			}
...
static errr Term_curs_win(int x, int y)
{
	...
	{
		if (x + 2 * arg_zoom - 1 < Term->wid && y + arg_zoom - 1 < Term->hgt)
		{
			if (Term->old->a[y][x + 2 * arg_zoom - 1] == 255)
			{
				rc.right += td->tile_wid * (2 * arg_zoom - 1);
				rc.bottom += td->tile_hgt * (arg_zoom - 1);
			}
		}
	}	
	else if (use_bigtile && x + 1 < Term->wid && Term->old->a[y][x + 1] == 255)
...
# ifdef USE_TRANSPARENCY
	...
	if ((arg_graphics == 2) || (arg_graphics == 3))
	{
		...
		if ((arg_graphics == 2) || (arg_graphics == 3))
		{
			...
			if ((w1 == tw2) && (h1 == th2))
			{
				...
				if (ecp[i] != 0 && eap[i] != 0)
				{
					...
					/* Mask out the tile */
					BitBlt(hdc, x2, y2, tw2, th2, hdcMask, x4, y4, SRCAND);

					/* Draw the tile */
					BitBlt(hdc, x2, y2, tw2, th2, hdcSrc, x4, y4, SRCPAINT);
				}
#endif
			}
			...
			else
			{
				...
				if (ecp[i] != 0 && eap[i] != 0)
				{
					...
					/* Mask out the tile */
					StretchBlt(hdc, x2, y2, tw2, th2, hdcMask, x4, y4, w1, h1, SRCAND);

					/* Draw the tile */
					StretchBlt(hdc, x2, y2, tw2, th2, hdcSrc, x4, y4, w1, h1, SRCPAINT);
				}
#endif
			}
		}
...
static void setup_menus(void)
{
	 ...
	 * Start TomeTik 0.3
	 */
	/* Menu "Window::Perfect Display" */
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		EnableMenuItem(hm, IDM_WINDOW_PERF_0 + i,
		               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);

		CheckMenuItem(hm, IDM_WINDOW_PERF_0 + i,
		              (data[i].perfect ? MF_CHECKED : MF_UNCHECKED));
	...
	for (i = 0; i < MAX_TERM_DATA; i++)
	{
		...
		if (data[i].visible && !data[i].perfect)
		{
			EnableMenuItem(hm, IDM_WINDOW_I_WID_0 + i,
			               MF_BYCOMMAND | MF_ENABLED);
	...
	EnableMenuItem(hm, IDM_OPTIONS_NEW_GRAPHICS,
	               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	EnableMenuItem(hm, IDM_OPTIONS_GERVAIS_GRAPHICS,
		       MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	EnableMenuItem(hm, IDM_OPTIONS_ASCII_GRAPHICS,
	               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	EnableMenuItem(hm, IDM_OPTIONS_ZOOM,
	               MF_BYCOMMAND | MF_DISABLED | MF_GRAYED);
	...
	CheckMenuItem(hm, IDM_OPTIONS_GERVAIS_GRAPHICS,
		   	      (arg_graphics == 3 ? MF_CHECKED : MF_UNCHECKED));
	CheckMenuItem(hm, IDM_OPTIONS_ASCII_GRAPHICS,
	              (arg_graphics == 0 ? MF_CHECKED : MF_UNCHECKED));
	CheckMenuItem(hm, IDM_OPTIONS_ZOOM,
	...
	/* Menu "Options", Item "Graphics" */
	EnableMenuItem(hm, IDM_OPTIONS_GERVAIS_GRAPHICS, MF_ENABLED);
	/* Menu "Options", Item "Graphics" */
	EnableMenuItem(hm, IDM_OPTIONS_ASCII_GRAPHICS, MF_ENABLED);
	/* Menu "Options", Item "Zoom" */
	EnableMenuItem(hm, IDM_OPTIONS_ZOOM, MF_ENABLED);
	/* Menu "Options", Item "Graphics" */
...
ofn.lStructSize = sizeof(OPENFILENAME);
	 ...
	 */
		...
		/* Perfect Display */
	case IDM_WINDOW_PERF_0:
	case IDM_WINDOW_PERF_1:
	case IDM_WINDOW_PERF_2:
	case IDM_WINDOW_PERF_3:
	case IDM_WINDOW_PERF_4:
	case IDM_WINDOW_PERF_5:
	case IDM_WINDOW_PERF_6:
	case IDM_WINDOW_PERF_7:
		{
			i = wCmd - IDM_WINDOW_PERF_0;

			if ((i < 0) || (i >= MAX_TERM_DATA)) break;

			td = &data[i];

			td->perfect = !td->perfect;
	 ...
	 * Start TomeTik 0.3
	 */
	case IDM_OPTIONS_GERVAIS_GRAPHICS:
		{
			/* Paranoia */
			if (!inkey_flag)
			{
