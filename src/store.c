...
static void display_entry(int pos)
{
	...
	if (show_store_graph) c_prt(get_item_letter_color(o_ptr), out_val, i * arg_zoom + 6, 0);
	else c_prt(get_item_letter_color(o_ptr), out_val, i + 6, 0);
...
/*			cur_col++; */
			if (a & 0x80)
			{
				...
				for (i1 = 0; use_bigtile ? i1 < 2 * arg_zoom : i1 < arg_zoom; i1++) 
					for (i2 = 0; i2 < arg_zoom; i2++)
						Term_draw(cur_col + i1, i * arg_zoom + 6 + i2, a2, c2); 
			}
			else
			{
				a2 = TERM_WHITE;
				c2 = ' ';
				for (i1 = 0; use_bigtile ? i1 < 2 * arg_zoom : i1 < arg_zoom; i1++) 
					Term_draw(cur_col + i1, i * arg_zoom + 6, a2, c2); 
		}
		}
		/* Draw car */
		if (o_ptr->k_idx) Term_draw(cur_col, i * arg_zoom + 6, a, c); 
		cur_col += 1 + arg_zoom * (use_bigtile ? 2 : 1);
	                ...
	                (st_info[st_ptr->st_idx].flags1 & SF1_MUSEUM))
	{
		...
		o_name[maxwid] = '\0';
		if (show_store_graph) c_put_str(tval_to_attr[o_ptr->tval], o_name, i * arg_zoom + 6, cur_col);
		else c_put_str(tval_to_attr[o_ptr->tval], o_name, i + 6, cur_col);
		...
		if (show_weights)
		{
			...
			if (show_store_graph) put_str(out_val, i * arg_zoom + 6, 68);
			else put_str(out_val, i + 6, 68);
		}
	}
	...
	else
	{
		...
		if (show_weights)
		{
			...
			if (show_store_graph) put_str(out_val, i * arg_zoom + 6, 61);
			else put_str(out_val, i + 6, 61);
		}
		...
		if (o_ptr->ident & (IDENT_FIXED))
		{
			...
			if (show_store_graph) c_put_str(color, out_val, i * arg_zoom + 6, 68);
			else c_put_str(color, out_val, i + 6, 68);
		}
...
static void display_inventory(void)
{
	...
	if (st_ptr->stock_num > 12)
	{
		...
		if (show_store_graph) prt("-more-", k * arg_zoom + 6, 3);
		else prt("-more-", k + 6, 3);
...
void store_prt_gold(void)
{
	...
	if (show_store_graph) prt("Gold Remaining: ", 7 + arg_zoom * 12, 53);
	else prt("Gold Remaining: ", 19, 53);

	strnfmt(out_val, 64, "%9ld", (long)p_ptr->au);
	if (show_store_graph) prt(out_val, 7 + arg_zoom * 12, 68);
	else prt(out_val, 19, 68);
}
	/*
	 * End TomeTik 0.3
	 */
...
void do_cmd_store(void)
{
	 ...
	 */
		/* Clear */
		clear_from(9 + arg_zoom * 12);


		/* Basic commands */
		if (show_store_graph) 
		{
			c_prt(TERM_YELLOW, " ESC.", 10 + arg_zoom * 12, 0);
			prt(") Exit.", 10 + arg_zoom * 12, 4);
		}
		else
		{
		...
		if (st_ptr->stock_num > 12)
		{
			if (show_store_graph) 
			{
				c_prt(TERM_YELLOW, " SPACE", 11 + arg_zoom * 12, 0);
				prt(") Next page", 11 + arg_zoom * 12, 6);
			}
			else
			{
		...
		/* Prompt */
		if (show_store_graph) prt("You may: ", 9 + arg_zoom * 12, 0);
		else prt("You may: ", 23, 0);
	/*
	 * End TomeTik 0.3
	 */
...
void do_cmd_home_trump(void)
{
	 ...
	 */
		/* Basic commands */
		if (show_store_graph) prt(" ESC) Exit from Building.", 10 + arg_zoom * 12, 0);
		else prt(" ESC) Exit from Building.", 22, 0);

		/* Browse if necessary */
		if (st_ptr->stock_num > 12)
		{
			if (show_store_graph) prt(" SPACE) Next page of stock", 11 + arg_zoom * 12, 0);
			else prt(" SPACE) Next page of stock", 23, 0);
		}
		...
		if (cur_store_num == 7)
		{
			if (show_store_graph) 
			{
				prt(" g) Get an item.", 10 + arg_zoom * 12, 31);
				prt(" d) Drop an item.", 11 + arg_zoom * 12, 31);
			}
			else
			{
		...
		else
		{
			if (show_store_graph) 
			{
				prt(" p) Purchase an item.", 10 + arg_zoom * 12, 31);
				prt(" s) Sell an item.", 11 + arg_zoom * 12, 31);
			}
			else
			{
		...
		if (show_store_graph) prt(" x) eXamine an item.", 10 + arg_zoom * 12, 56);
		else prt(" x) eXamine an item.", 22, 56);
