...
void reset_visuals(void)
{
	 ...
	 */
		 ...
		 * 
		 */
		else if (streq(ANGBAND_GRAF, "gervais"))
		{
...
void show_inven_aux(bool mirror, bool everything)
{
	...
	if (show_inven_graph) lim -= (use_bigtile ? 4 : 2) * arg_zoom;
	...
	for (k = 0, i = 0; i < z; i++)
	{
		...
		if (show_inven_graph) l += (use_bigtile ? 4 : 2) * arg_zoom;
	...
	for (j = 0; j < k; j++)
	{
		...
		/* Clear the line */
		if (use_zoom && show_inven_graph)
		{
			prt("", row + j * arg_zoom, col ? col - 2 : col); 
			prt("", row + j * arg_zoom + 1, col ? col - 2 : col); 
		}
		else prt("", row + j, col ? col - 2 : col);
		...
		if (use_zoom && show_inven_graph)
		c_put_str(get_item_letter_color(o_ptr), tmp_val, row + j * arg_zoom, col);
		else c_put_str(get_item_letter_color(o_ptr), tmp_val, row + j, col);
...
/*			if (!o_ptr->k_idx) c = ' '; */
			...
			/* Clear area before draw car */
			for (i1 = 0; use_bigtile ? i1 < 2 * arg_zoom : i1 < arg_zoom; i1++) 
				for (i2 = 0; i2 < arg_zoom; i2++)
					Term_draw(col + 3 + i1, row + j * arg_zoom + i2, a2, c2); 
		}

			/* Draw car */
			if (o_ptr->k_idx) Term_draw(col + 3, row + j * arg_zoom, a, c);
		}

		/* Display the entry itself */
		if (use_zoom && show_inven_graph)
		{
			text_out_indent = col + 4 + arg_zoom * 2;
			if (show_weights) text_out_wrap = wid - 8;
			else text_out_wrap = 0;
			Term_gotoxy(text_out_indent, row + j * arg_zoom);
			text_out_c(out_color[j], out_desc[j]);
			
			/* Restore indent and wrap factors */
			text_out_indent = 0;
			text_out_wrap = 0;
		}
		else 
		...
		if (show_weights)
		{
			...
			{ 
				if (use_zoom && show_inven_graph)
					put_str(tmp_val, row + j * arg_zoom, wid - 8);
				else
					put_str(tmp_val, row + j, wid - 8);
			}
			else
			{
				if (use_zoom && show_inven_graph)
					put_str(tmp_val, row + j * arg_zoom, wid - 9 - ((wid - 2) % (arg_zoom *(use_bigtile ? 2 : 1))));
				else put_str(tmp_val, row + j, wid - 9);
			}
		}
	}
	...
	if (mirror)
	{
		...
		if ((use_zoom || use_bigtile) && show_inven_graph)
			for (j = row + arg_zoom * k; j < Term->hgt; j++) Term_erase(0, j, 255);
		else
			for (j = row + k; j < Term->hgt; j++) Term_erase(0, j, 255);
	}
	...
	else
	{
		...
		if (j && (j < Term->hgt - 1)) 
		{
			if ((use_zoom || use_bigtile) && show_inven_graph)
			{
				if (row + j * arg_zoom < Term->hgt - 1) 
					prt("", row + j * arg_zoom, col ? col - 2 : col);
			}
			else if (row + j < Term->hgt - 1)
...
void show_equip_aux(bool mirror, bool everything)
{
	...
	/* Require space for weight (if needed) */
	if (show_weights && !(show_labels && show_equip_graph && use_zoom)) lim -= 9;

	if (show_equip_graph && use_zoom) lim -= (use_bigtile ? 2 : 1) * arg_zoom;
	...
	for (k = 0, i = INVEN_WIELD; i < INVEN_TOTAL; i++)
	{
		...
		/* Extract the maximal length (see below) */
			if (show_equip_graph && use_zoom)
				l = strlen(out_desc[k]) / arg_zoom + (2 + 3);
			else
		...
		/* Increase length for weight (if needed) */
			if (show_weights && !(show_labels && show_equip_graph && use_zoom)) l += 9;

			if (show_equip_graph) l += (use_bigtile ? 2 : 1) * arg_zoom;
...
/*			if (!o_ptr->k_idx) c = ' '; */
			...
			/* Clear area before draw char */
			for (i1 = 0; use_bigtile ? i1 < 2 * arg_zoom : i1 < arg_zoom; i1++) 
				for (i2 = 0; i2 < arg_zoom; i2++)
					Term_draw(col + 2 + i1, row + j * arg_zoom + i2, a2, c2); 
			}

			/* Draw char */
			if (o_ptr->k_idx) Term_draw(col + 2, row + j * arg_zoom, a, c);
		...
		if (show_labels)
		{
			...
			if (show_equip_graph)
			{
				put_str(tmp_val, row + j * arg_zoom, col + 2 + arg_zoom * (use_bigtile ? 2 : 1));
				colx = col + 18 + arg_zoom * (use_bigtile ? 2 : 1);
				rowy = row + j * arg_zoom;
			}
			else
			{
		...
		else
		{
			if (show_equip_graph)
			{
				colx = col + 2 + arg_zoom * (use_bigtile ? 2 : 1);
				rowy = row + j * arg_zoom;
			}
			else
			{
		...
		text_out_indent = colx;
		if (!show_weights || (show_weights && show_labels && show_equip_graph && use_zoom))
			text_out_wrap = 0;
		else
			text_out_wrap = wid - 8;
		
			/* Display the entry itself */
		Term_gotoxy(colx, rowy);
		text_out_c(out_color[j], out_desc[j]);
		...
		if (show_weights)
		{
			...
			if (mirror) 
			{
				if (show_labels && show_equip_graph && use_zoom)
				{
					(void)sprintf(tmp_val, "(%d.%d lb)", wgt / 10, wgt % 10);
					colx = 2 + arg_zoom * (use_bigtile ? 2 : 1);
					rowy = row + j * arg_zoom + 1;
				}
				else
				{
			(void)sprintf(tmp_val, "%3d.%d lb", wgt / 10, wgt % 10);
					colx = wid - 8;
					if (show_equip_graph && use_zoom) rowy = row + j * arg_zoom;
					else rowy = row + j;
				}
			}
			else 
	...
	if (mirror)
	{
		...
		for (j = row + k; j < Term->hgt; j++)
		{
			...
			if (show_equip_graph)
			{
				Term_erase(0, j * arg_zoom, 255);
				if (use_zoom) Term_erase(0, j * arg_zoom + 1, 255);
			}
			else Term_erase(0, j, 255);
		}
	}
	...
	else
	{
		...
		if (j && (j < 23)) 
		{
			if (show_equip_graph) prt("", row + j * arg_zoom, col ? col - 2 : col);
			else prt("", row + j, col ? col - 2 : col);
		}
	}
}
