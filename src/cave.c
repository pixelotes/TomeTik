...
/*
 * TomeTik -- Gervais terrain edging (a higher terrain overhanging a lower
 * neighbour with a soft organic border).  Full opaque tiles drawn from the
 * game's own base terrain, packed in 32x32.bmp, 12 slots each (char 0xB2+slot):
 *   grass  <-> shallow water : row 1 (attr 0x81)
 *   grass  <-> dirt          : row 3 (attr 0x83)
 *   cobble <-> dirt          : row 4 (attr 0x84)
 * Slots: 0 N, 1 S, 2 W, 3 E edge;  4 NW, 5 NE, 6 SW, 7 SE outer corner;
 *        8..11 inner (concave) corners by the diagonal holding the lower terrain.
 */
#define EDGE_CHAR    0xB2
...
static int terrain_class_at(int y, int x)
{
	int f;

	if ((y < 0) || (x < 0) || (y >= cur_hgt) || (x >= cur_wid)) return TC_OTHER;

	f = cave[y][x].mimic ? cave[y][x].mimic : cave[y][x].feat;
	return terrain_class(f);
}

/*
 * Slot for a cell whose neighbours of class `low` define the edge, or -1 if
 * none.  Outer corners first, then edges, then concave (inner) corners.
 */
static int edge_slot_low(int y, int x, int low)
{
	bool n = (terrain_class_at(y - 1, x) == low);
	bool s = (terrain_class_at(y + 1, x) == low);
	bool e = (terrain_class_at(y, x + 1) == low);
	bool w = (terrain_class_at(y, x - 1) == low);
	...
	if (terrain_class_at(y + 1, x + 1) == low) return 8;   /* SE */
	if (terrain_class_at(y + 1, x - 1) == low) return 9;   /* SW */
	if (terrain_class_at(y - 1, x + 1) == low) return 10;  /* NE */
	if (terrain_class_at(y - 1, x - 1) == low) return 11;  /* NW */
...
#endif /* USE_TRANSPARENCY */
{
	...
	bool graf_new = ((graphics_mode == GRAPHICS_ISO) ||
	                 ...
	                 (graphics_mode == GRAPHICS_GERVAIS));
	/*
	 * End TomeTik 0.3
	 */
	...
	*cp = c;
	...
	/*
	 * TomeTik -- terrain edging.  Full opaque tile substitution: a cell whose
	 * terrain overhangs a lower neighbour uses the matching soft-edge tile.
	 * grass over shallow water (priority) or dirt; cobblestone over dirt.
	 */
	if (gervais_terrain_edge && (graphics_mode == GRAPHICS_GERVAIS) &&
	    (info & (CAVE_MARK | CAVE_SEEN)) && !c_ptr->m_idx && !c_ptr->o_idx)
	{
		int tc = terrain_class(feat);
		int sl = -1;
		byte ea = 0;
		...
		{
			sl = edge_slot_low(y, x, TC_DIRT);
			if (sl >= 0) ea = CDEDGE_ATTR;
		}

		if (sl >= 0)
		{
			*tap = *ap = ea;
			*tcp = *cp = (char)(EDGE_CHAR + sl);
		}
	}
	 ...
	 */
				if (player_char_health && 
					((graphics_mode == GRAPHICS_NEW) || (graphics_mode == GRAPHICS_GERVAIS)))
	/*
	 * End TomeTik 0.3
	 */
				{
...
static int panel_col_of(int col)
{
	...
	if (use_zoom) col *= arg_zoom;		
	return col + COL_MAP;
}

/*
 * Calculate panel row of a location in the map
 */
static int panel_row_of(int row)
{
	row -= panel_row_min;
	if (use_zoom) row *= arg_zoom;		
	return row + ROW_MAP;
}
	/*
	 * End TomeTik 0.3
...
void move_cursor_relative(int row, int col)
{
	 ...
	 * Start TomeTik 0.3
	 */
	Term_gotoxy(panel_col_of(col), panel_row_of(row));
	/*
	 * End TomeTik 0.3
	 */
}
	...
	if ((use_bigtile) || (use_zoom))
	{
		...
		for (i = 0; use_bigtile ? i < 2 * arg_zoom : i < arg_zoom; i++) 
			for (j = 0; j < arg_zoom; j++)
				if (i != 0 || j != 0) Term_draw(colx + i, rowy + j, a2, c2);
		}
		else
		{
			a2 = TERM_WHITE;
			c2 = ' ';
			for (i = 1; use_bigtile ? i < 2 * arg_zoom : i < arg_zoom; i++)
				Term_draw(colx + i, rowy, a2, c2);
		}
	}
	/* Draw the char using the attr */
	Term_draw(colx, rowy, a, c);
}
	/*
	 * End TomeTik 0.3
	 */
...
#ifdef USE_TRANSPARENCY
#ifdef USE_EGO_GRAPHICS
		Term_queue_char(colx, rowy, a, c, ta, tc, ea, ec);
# else /* USE_EGO_GRAPHICS */
		Term_queue_char(colx, rowy, a, c, ta, tc);
# endif  /* USE_EGO_GRAPHICS */
#else /* USE_TRANSPARENCY */
		Term_queue_char(colx, rowy, a, c);
#endif /* USE_TRANSPARENCY */
...
void display_map(int *cy, int *cx)
{
	...
	for (i = 0; i < hgt; i++)
	{
		C_MAKE(ma[i], wid, byte);
		C_MAKE(mc[i], wid, char);
		C_MAKE(mp[i], wid, byte);
	}
...
# endif  /* USE_EGO_GRAPHICS */
			...
			if ((p_ptr->py == j) && (p_ptr->px == i)) 
			{
				tp = 255;
				/* Player location in dungeon */
				*cx = use_bigtile ? 2 * x : x;
				*cy = y;
			}
	...
	for (y = 0; y < hgt; y++)
	{
		...
		for (x = 0; x < wid; x++)
		{
			...
			/* Double width tile mode and zoom mode requires filler demp */
			if (use_bigtile || use_zoom)
			{
				byte a2;
				char c2;
				int colx = x * (use_bigtile ? 2 * arg_zoom : arg_zoom);
				int rowy = y * arg_zoom;
				...
				Term_draw(colx, rowy, ta, tc); 
				}

			/* Add the character */
			else Term_draw(x, y, ta, tc); 
	...
	for (i = 0; i < hgt; i++)
	{
		C_FREE(ma[i], wid, byte);
		C_FREE(mc[i], wid, char);
		C_FREE(mp[i], wid, byte);
	}
...
void do_cmd_view_map(void)
{
	...
	/* Backup zoom parameters */
	bool use_zoom_temp = use_zoom;
	int arg_zoom_temp = arg_zoom;
	...
	/* Wait for it */
	put_str("Hit any key to continue", hgt - 1, MAX((wid - 23) / 2,0));
	...
	/* Restore zoom parameters */
	use_zoom = use_zoom_temp;
	arg_zoom = arg_zoom_temp;
