...
void show_building(store_type *s_ptr)
{
	...
	for (i = 0; i < 6; i++)
	{
		...
		if (ba_ptr->letter != '.')
		{
			...
			if (show_store_graph) 
				c_put_str(TERM_YELLOW, tmp_str, 9 + arg_zoom * 12 + (i / 2), 17 + (30 * (i % 2)));
			else c_put_str(TERM_YELLOW, tmp_str, 21 + (i / 2), 17 + (30 * (i % 2)));

			strnfmt(tmp_str, 80, ") %s %s", ba_ptr->name + ba_name, buff);
			if (show_store_graph) 
				c_put_str(action_color, tmp_str, 9 + arg_zoom * 12 + (i / 2), 2 + 17 + (30 * (i % 2)));
			else c_put_str(action_color, tmp_str, 21 + (i / 2), 2 + 17 + (30 * (i % 2)));
		}
	}
}
